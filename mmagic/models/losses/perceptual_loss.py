...
class ContrastiveLoss1(nn.Module):
    """Contrastive Loss.
    paper: Wu_Contrastive_Learning_for_Compact_Single_Image_Dehazing_CVPR_2021_paper.pdf
    """
    ...
    def forward(self, anchor, positive, negative):
        ...
        Args:
            anchor (Tensor): Restored image with shape (n, c, h, w).
            positive (Tensor): Ground-truth image with shape (n, c, h, w).
            positive (Tensor): Degraded image with shape (n, c, h, w).
        ...
        if self.norm_img:
            anchor = (anchor + 1.) * 0.5
            positive = (positive + 1.) * 0.5
            negative = (negative + 1.) * 0.5
        # extract vgg features
        anchor_features = self.vgg(anchor)
        positive_features = self.vgg(positive.detach())
        negative_features = self.vgg(negative.detach())

        # calculate perceptual loss
        assert self.perceptual_weight > 0, 'perceptual weight must greater than 0.'

        contrastive_loss = 0
        pull_an_po = 0
        push_an_ne = 0
        for k in negative_features.keys():
            pull_an_po = self.criterion(
                anchor_features[k], positive_features[k])
            push_an_ne = self.criterion(
                anchor_features[k], negative_features[k])
            contrastive = pull_an_po / (push_an_ne + 1e-7)

            contrastive_loss += contrastive * self.layer_weights[k]

        contrastive_loss *= self.perceptual_weight
