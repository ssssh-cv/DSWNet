...
class BaseUWModel(BaseModel):
    ...
    def __init__(self,
        ...
        self.ssim_loss = MODELS.build(ssim_loss) if ssim_loss else None
        self.perceptual_loss = MODELS.build(perceptual_loss) if perceptual_loss else None
        self.contrastive_loss = MODELS.build(contrastive_loss) if contrastive_loss else None
    ...
    def forward_train(self,
        ...
        # print(f'========== {torch.max(feats)}  {torch.max(inputs)}  {torch.max(batch_gt_data)}')

        losses = dict()

        losses['loss_pixel'] = self.pixel_loss(feats, batch_gt_data)
        
        if self.ssim_loss:
            losses['loss_ssim'] = self.ssim_loss(feats, batch_gt_data)

        if self.perceptual_loss:
            loss_percep, loss_style = self.perceptual_loss(feats, batch_gt_data)
        ...
        if self.contrastive_loss:
            losses['loss_contrastive'] = self.contrastive_loss(
                                            anchor=feats,
                                            positive=batch_gt_data,
                                            negative=inputs)
